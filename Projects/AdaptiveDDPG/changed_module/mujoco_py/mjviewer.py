...
class MjViewer(MjViewerBasic):
    ...
    def render(self):
        ...
        if self._paused:
            ...
            self._loop_count += self.sim.env.opt.timestep * self.sim.nsubsteps / \
    ...
    def _create_full_overlay(self):
        ...
        step = round(self.sim.data.time / self.sim.env.opt.timestep)
        self.add_overlay(const.GRID_BOTTOMRIGHT, "Step", str(step))
        self.add_overlay(const.GRID_BOTTOMRIGHT, "timestep", "%.5f" % self.sim.env.opt.timestep)
    ...
    def key_callback(self, window, key, scancode, action, mods):
        ...
        # Advances simulation by one step.
            ...
            if self._transparent:
                self.sim.env.geom_rgba[:, 3] /= 5.0
            else:
                self.sim.env.geom_rgba[:, 3] *= 5.0
        elif key == glfw.KEY_M:  # Shows / hides mocap bodies
            self._show_mocap = not self._show_mocap
            for body_idx1, val in enumerate(self.sim.env.body_mocapid):
                if val != -1:
                    for geom_idx, body_idx2 in enumerate(self.sim.env.geom_bodyid):
                        if body_idx1 == body_idx2:
                            if not self._show_mocap:
                                # Store transparency for later to show it.
                                self.sim.extras[
                                    geom_idx] = self.sim.env.geom_rgba[geom_idx, 3]
                                self.sim.env.geom_rgba[geom_idx, 3] = 0
                            else:
                                self.sim.env.geom_rgba[
