...
earth = as_obj(pos=vector(earth_orbit['r'] - cm_dis, 0, 0), m=mass["earth"], radius=radius["earth"], make_trail=False, color=color.blue)
moon = as_obj(pos=vector(earth_orbit['r'] + moon_orbit['r'] - cm_dis, 0, 0), m=mass["moon"], radius=radius["moon"], make_trail=False)

moon.v = vec(0, 0, -earth_orbit['v'] - moon_orbit['v'])
earth.v = vec(0, 0, -earth_orbit['v'] + moon_orbit['v'] * moon.m / earth.m)

moon.pos -= vec((1 - cos(theta)) * (moon_orbit['r'] - cm_dis), sin(theta) * (moon_orbit['r'] - cm_dis), 0)
earth.pos += vec((1 - cos(theta)) * cm_dis, sin(theta) * cm_dis, 0)
