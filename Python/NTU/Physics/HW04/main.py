...
oscillation = graph(width=400, align='left')
funct1 = gdots(graph=oscillation, color=color.blue, width=4)
...
def calculation(n):
    ...
    while True:
        ...
        ball_v[1:-1] += ((spring_len[1:-1] - d) * k/m - (spring_len[:-2] - d) * k/m) * dt
        ball_v[-1] += ((spring_len[-1] - d) * k/m - (spring_len[-2] - d) * k/m) * dt
        ball_pos += ball_v*dt
