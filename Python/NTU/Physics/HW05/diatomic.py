...
class CO_molecule:
    ...
    def v_K(self):  # return kinetic energy of the vibration motion
        return (0.5 * self.C.m * dot(self.C.v - self.com_v(), self.bond.axis) ** 2 +
                0.5 * self.O.m * dot(self.O.v - self.com_v(), self.bond.axis) ** 2) / mag2(self.bond.axis)

    def r_K(self):  # return kinetic energy of the rotational motion
        return (0.5 * mag2(cross(self.C.v - self.com_v(), self.bond.axis)) * self.C.m +
                0.5 * mag2(cross(self.O.v - self.com_v(), self.bond.axis)) * self.O.m) / mag2(self.bond.axis)
