...
def change_stage():
    global stage, v_W, observation, container
    if stage == 0:
        v_W = L / (20000.0 * dt)
    if stage == 1:
        v_W = 0
        ...
        for v in arange(0., 4201. + dv, dv):  # theoretical speed distribution
            theory_low_T.plot(pos=(v, (deltav / dv) * N * 4. * pi * (
                    (m / (2. * pi * k * (T * 2**(2/3)))) ** 1.5) * exp(
                (-0.5 * m * v ** 2) / (k * (T * 2**(2/3)))) * (v ** 2) * dv))
    if stage == 2:
...
def keyinput(evt):
    global stage, v_W, observation
    if evt.key == 'n' and stage < 3:
        change_stage()
...
while True:
    ...
    if stage == 1 and container.length <= L:
        change_stage()
    if stage != 1:
