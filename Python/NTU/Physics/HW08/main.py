...
while True:
    ...
    for i in range(N):
        if random() <= prob:
            total_c += 1
            v_ = mag(a_to_v(v_array[i])) * norm(a_to_v(np.random.rand(3) - 0.5))
            v_array[i] = [v_.x, v_.y, v_.z]
    ...
    if int(t/dt)%2000 == 0:
        tau = t*N/total_c
        """
        The simulated drift velocity and the real one are quite close. 
        Both of them are roughly within 1900 and 2000 m/s.
        """
